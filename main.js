...
function animateCard(direction, updateFn) {
    wordCard.classList.add(direction === 'next' ? 'exit-left' : 'exit-right');

    setTimeout(() => {
        updateFn();
        wordCard.className = 'word-card hidden';
...
if (prevBtn) {
    ...
    function initializeCharts() {
        ...
        try {
            ...
            const savedWordsCount = wordList.length;
            ...
            barChart = new Chart(barCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['#6a0dad', '#9932cc', '#8a2be2'],
                        borderWidth: 1
                    }]
                },
                options: {
...
if (hamburger && mobileMenu) {
    ...
    mobileMenu.querySelectorAll('li').forEach(item => {
        item.addEventListener('click', () => {
            ...
            const navItem = document.querySelector(`nav ul li[data-page="${targetPage}"]`);
...
function animateWord(direction, updateFn) {
  wordCard.classList.add(direction === "next" ? "exit-left" : "exit-right");
